...
def log_debug_info(filename, line_no, entry, debug_info, reverse_vocab_table):
  ...
  if json_elem:
    ...
    for i in range(len(json_elem["annotations"])):
        logging.info(
            "json.annotations[%d].minimal_answer.plaintext_start_byte: %d",i,
            json_elem["annotations"][i]["minimal_answer"]["plaintext_start_byte"])
        logging.info(
            "json.annotations[%d].minimal_answer.plaintext_end_byte: %d",i,
            json_elem["annotations"][i]["minimal_answer"]["plaintext_end_byte"])
