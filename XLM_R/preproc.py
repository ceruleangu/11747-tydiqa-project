...
def convert_examples_to_features(tydi_examples, is_training, vocab_file,
  ...
  tokenizer = tokenization.TyDiXLMTokenizer(vocab_file=vocab_file)
  num_examples = 0
...
def convert_single_example(tydi_example,
  ...
  #print(contexts_wordpieces, contexts_start_offsets, contexts_end_offsets,
   #contexts_offset_to_wp)
  #print(tydi_example.contexts)
  # Check invariants.
  ...
  #print('this is america', contexts_start_offsets)
  ...
  #print('asdfasdf'*4, wp_start_offsets)
  #print('this is china', tydi_example.context_to_plaintext_offset)
  ...
  # 2. 2*[SEP] -- Special separator token, placed after question.
  # 3. [SEP] -- Special separator token, placed after article content.

  max_wordpieces_for_doc = max_seq_length - len(question_wordpieces) - 4
  ...
  for (doc_span_index, doc_span) in enumerate(doc_spans):
    ...
    for i in range(doc_span.length):
      ...
      #xlm-roberta doesn't make use of segment type ids so a list of 1 will be returned
      segment_ids.append(0)
    wps.append(tokenizer.get_vocab_id("[SEP]"))
    wp_start_offset.append(-1)
    wp_end_offset.append(-1)
    segment_ids.append(0)
    #print(len(wps), len(segment_ids))
    ...
    if is_training:
      ...
      if ((not contains_an_annotation) or
        ...
        doc_offset = len(question_wordpieces) + 3  #  one for CLS, one for SEP. #for xlm, two for [SEP]
        ...
        answer_start_byte_offset = wp_start_offset[start_position-1]
        #print('start_position' * 100, wp_start_offset[start_position - 2:start_position+2])
        answer_end_byte_offset = wp_end_offset[end_position-1]
        #print('start_position'*4, answer_start_byte_offset)
        #print('end_position' * 5, wp_end_offset[end_position-2:end_position+2])
        #print('this is original text start positions', contexts_wordpieces[wp_start_position:wp_end_position])
        #print('this is context start positions', wps[start_position:end_position])
        answer_text = tydi_example.contexts[
            answer_start_byte_offset:answer_end_byte_offset]
        #print(wp_start_offset, 'end*'*100)
        #print(len(wp_start_offset), '*' * 100)
        print(tokenizer.huggingface.decode(wps[start_position : end_position + 1]))
        print('start position', wp_start_offset[start_position])
        print('end_position ', wp_end_offset[end_position])
...
def create_mapping(start_offsets, end_offsets, context_to_plaintext_offset):
  ...
  plaintext_start_offsets = [
      context_to_plaintext_offset[i] + 1 if i >= 0 else -1 for i in start_offsets
  ]
  plaintext_end_offsets = [
      context_to_plaintext_offset[i] + 1 if i >= 0 else -1 for i in end_offsets
  ]
