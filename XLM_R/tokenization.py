...
class TyDiXLMTokenizer(object):
  ...
  def tokenize_with_offsets(self, text):
    ...
    start_offsets = [i.begin for i in list(self.spt.pieces)]
    end_offsets = [i.end - 1 if i.end - 1 >= 0 else 0 for i in list(self.spt.pieces)]
    offset_to_bytes = {}
    for i in range(len(start_offsets)):
      for j in range(start_offsets[i], end_offsets[i] + 1):
        offset_to_bytes[j] = i
    bytes_out = self.huggingface.encode(text)[1:-1]
    #assert len(bytes_out) == len(start_offsets)
    #assert len(bytes_out) == len(end_offsets)
    #assert len(bytes_out) == len(offset_to_bytes)

    return bytes_out, start_offsets, end_offsets, offset_to_bytes

  def get_vocab_id(self, sepcial_token):
    return self.huggingface._convert_token_to_id(sepcial_token)

  def vocab_size(self):
    return self.huggingface.vocab_size()

  def tokenize(self, text):
    return self.huggingface.encode(text)[1:-1]
