...
class BertAttention(nn.Module):
    ...
    def prune_heads(self, heads):
        if not heads:
            return
        masks = torch.ones(self.self.num_attention_heads, self.self.attention_head_size)
        heads = set(heads) - self.pruned_heads
        for head in heads:

            head = head - sum(1 if h < head else 0 for h in self.pruned_heads)
            masks[head] = 0
        masks = masks.view(-1).contiguous().eq(1)
        index = torch.arange(len(masks))[masks].long()
    ...
    def forward(
        ...
        self_outputs = self.selfsttention(
...
class BertModel(BertPreTrainedModel):
    ...
    def forward(self,input_ids=None,attention_mask=None,
        ...
        encoder_outputs = self.encoder.foward(
            embedding_output,
